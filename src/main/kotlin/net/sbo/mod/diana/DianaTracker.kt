...
import net.sbo.mod.utils.game.Mayor.getMayor
import net.sbo.mod.utils.game.World.isInSkyblock
import java.util.regex.Pattern

object DianaTracker {
    private val rareDrops = mapOf<String, String>(
        "DWARF_TURTLE_SHELMET" to "§9",
        "CROCHET_TIGER_PLUSHIE" to "§5",
        "ANTIQUE_REMEDIES" to "§5",
        "MINOS_RELIC" to "§5"
    )
    ...
    private val forbiddenCoins = listOf(
        1L,
        5L,
        20L,
        1000L,
    ...
    fun init() {
        ...
        Register.onChatMessageCancelable(
            Pattern.compile(
                "(.*?) §efound a §cPhoenix §epet!(.*?)$",
                Pattern.DOTALL
            )
        ) { message, matchResult ->
            if (QOL.phoenixAnnouncer) {
                Chat.chat("§6[SBO] §cGG §eFound a §cPhoenix ${"§e"}pet!")
                Helper.showTitle("§c§lPhoenix Pet!", "", 0, 25, 35)
            }
            if (Helper.getSecondsPassed(lastDianaMobDeath) > 2) return@onChatMessageCancelable true
            val player = matchResult.group(2).removeFormatting()
            if (Player.getName() != Helper.getPlayerName(player)) return@onChatMessageCancelable true
            sleep(1000) {
        ...
        if (item.itemId in rareDrops.keys) {
            ...
            if (item.itemId == "MINOS_RELIC") {
                ...
                if (Diana.lootAnnouncerScreen) {
                    val subTitle =
                        if (Diana.lootAnnouncerPrice) "§6${Helper.getItemPriceFormatted(item.itemId)} coins" else ""
    ...
    fun trackRngDropsWithChat() {
        Register.onChatMessageCancelable(
            ...
            if (!checkDiana()) return@onChatMessageCancelable true
            ...
            when (drop.substring(2, 16)) {
                "Enchanted Book" -> {
                    if (!drop.contains("Chimera")) return@onChatMessageCancelable true

                    playCustomSound(Customization.chimSound[0], Customization.chimVolume)
                    if (Diana.lootAnnouncerScreen) {
                        val subTitle =
                            if (Diana.lootAnnouncerPrice) "§6${Helper.getItemPriceFormatted("CHIMERA")} coins" else ""
                ...
                "Daedalus Stick" -> {
                    ...
                    if (Diana.lootAnnouncerScreen) {
                        val subTitle =
                            if (Diana.lootAnnouncerPrice) "§6${Helper.getItemPriceFormatted("DAEDALUS_STICK")} coins" else ""
